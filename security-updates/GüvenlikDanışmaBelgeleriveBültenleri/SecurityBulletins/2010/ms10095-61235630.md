---
TOCTitle: 'MS10-095'
Title: 'Microsoft Güvenlik Bülteni MS10-095 - Önemli'
ms:assetid: 'ms10-095'
ms:contentKeyID: 61235630
ms:mtpsurl: 'https://technet.microsoft.com/tr-TR/library/ms10-095(v=Security.10)'
---

Security Bulletin

Microsoft Güvenlik Bülteni MS10-095 - Önemli
============================================

Microsoft Windows'daki Güvenlik Açığı Uzaktan Kod Yürütülmesine İzin Verebilir (2385678)
----------------------------------------------------------------------------------------
...
|--------------------------------------------------------------------------------------------------------------------------------------------------------|---------------------------|----------------------|---------------------------------------------|
| [32-bit sistemler için Windows 7](http://www.microsoft.com/downloads/details.aspx?familyid=75591d37-2cb8-4cdf-acbb-89cd0d1a9290)                       | Uzaktan Kod Yürütme       | Önemli               | Yok                                         |
| [x64 tabanlı sistemler için Windows 7](http://www.microsoft.com/downloads/details.aspx?familyid=3a0c4dd0-98b4-4e7a-99ed-22b9d9f76cd1)                  | Uzaktan Kod Yürütme       | Önemli               | Yok                                         |
| [x64 tabanlı sistemler için Windows Server 2008 R2](http://www.microsoft.com/downloads/details.aspx?familyid=f58a765f-cea9-456d-b0ab-bfc70b109cbf)\*   | Uzaktan Kod Yürütme       | Önemli               | Yok                                         |
| [Itanium tabanlı sistemler için Windows Server 2008 R2](http://www.microsoft.com/downloads/details.aspx?familyid=13a9f838-ac07-43dc-9aee-a77207998e1e) | Uzaktan Kod Yürütme       | Önemli               | Yok                                         |
...
<th style="border:1px solid black;" >BranchCache'de Güvenli Olmayan Şekilde Kitaplık Yükleme Güvenlik Açığı - CVE-2010-3966</th>
...
BranchCache'de Güvenli Olmayan Şekilde Kitaplık Yükleme Güvenlik Açığı - CVE-2010-3966  
--------------------------------------------------------------------------------------
  
<span></span>
BranchCache işlevselliğinin desteklenmediği platformlardaki belirli dosyaların Microsoft Windows tarafından açılma biçiminde bir uzaktan kod yürütme güvenlik açığı bulunmaktadır. Bu açığı başarıyla kullanan bir saldırgan, etkilenen sistemin tüm denetimini ele geçirebilir. Saldırgan, program yükleyebilir; verileri görüntüleyebilir, değiştirebilir veya silebilir; tüm kullanıcı haklarına sahip olan yeni hesaplar oluşturabilir. Hesapları, sistemde az sayıda kullanıcı hakkıyla yapılandırılmış olan kullanıcılar, yönetici haklarıyla çalışan kullanıcılardan daha az etkilenebilir.
  
Bu güvenlik açığını Ortak Güvenlik Açıkları ve Etkilenmeler listesinde standart girdi olarak görüntülemek için, bkz: [CVE-2010-3966](http://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2010-3966).
  
#### BranchCache'de Güvenli Olmayan Şekilde Kitaplık Yükleme Güvenlik Açığı - CVE-2010-3966 için Azaltıcı Etkenler
...
#### BranchCache'de Güvenli Olmayan Şekilde Kitaplık Yükleme Güvenlik Açığı - CVE-2010-3966 için Geçici Çözümler
...
#### BranchCache'de Güvenli Olmayan Şekilde Kitaplık Yükleme Güvenlik Açığı - CVE-2010-3966 Hakkında SSS
...
Bu güvenlik açığı, Windows BranchCache tarafından kullanılan belirli bir DLL dosyasının sistemde bulunup bulunmadığını Windows doğrulamaya çalıştığında ortaya çıkar. Varsayılan DLL dosyası sistemde bulunuyorsa, bu ikili dosya öncelikle Windows klasör yapısındaki güvenli ve yerel konumlarda aranır. Ancak yoksa, uygulama bu dosyayı açıldığı konumda bulmaya çalışır ve bu konum bir ağ paylaşımı gibi güvenli olmayan bir konum olabilir. Başka bir konuma yerleştirilen özel hazırlanmış bir dosya, Windows'un saldırgan tarafından seçilen bir kodu oturum açmış kullanıcının bağlamında yürütmesine neden olabilir.
  
Windows BranchCache tarafından yüklenen kitaplık yalnızca BranchCache desteğiyle yayımlanmış olan Windows sürümlerinde bulunmaktadır. Bu platformlar Windows 7 Enterprise ve Ultimate ile Windows Server 2008 R2'dir. Diğer Windows 7 sürümlerinde bu ikili dosya sistemde ilgili bir konumda bulunmamaktadır ve dolayısıyla bu sürümler varsayılan olarak bu açıktan etkilenir. BranchCache DLL dosyası varsayılan konumda bulunmuyorsa tüm Windows 7 ve Windows Server 2008 R2 sürümleri bu açıktan etkilenebilirler.
  
**Windows BranchCache nedir?**    
BranchCache, Windows Server 2008 R2 ve Windows 7 işletim sistemlerinin bazı sürümlerinde bulunan bir geniş ağ (WAN) bant genişliği iyileştirme teknolojisidir. WAN bant genişliğini en iyi duruma getirmek için, BranchCache merkez ofis içerik sunucularınızdaki içeriği kopyalar ve şube ofislerde önbelleğe alır; böylece, şubelerdeki istemci bilgisayarların içeriğe WAN üzerinden değil, yerel olarak erişebilmeleri sağlanır. Daha fazla bilgi için, bkz: [Windows Server 2008 R2 ve BranchCache](http://www.microsoft.com/windowsserver2008/en/us/branch-cache.aspx).
...
Saldırgan bir kullanıcıyı özel hazırlanmış bir dinamik bağlantı kitaplığı (DLL) dosyasıyla aynı ağ dizininde bulunan ve belirli Windows uygulamaları tarafından açılan .eml ve .rss (Windows Live Mail) ya da .wpost (Microsoft Live Yazar) gibi meşru bir dosya türünü açmaya ikna edebilir. Daha sonra, meşru dosyanın açıldığı sırada BranchCache bu DLL dosyasını yüklemeyi ve içerdiği kodu yürütmeyi deneyebilir.
  
E-posta tabanlı saldırı senaryosunda, saldırgan bir kullanıcıya meşru bir .eml, .rss veya .wpost eki gönderip kullanıcıyı bu eki özel hazırlanmış bir DLL dosyası içeren bir dizine yerleştirmeye ve meşru dosyayı açmaya ikna ederek bu güvenlik açığından yararlanabilir. Daha sonra, meşru dosyanın açıldığı sırada Windows Live Mail veya Microsoft Live Yazar bu DLL dosyasını yüklemeyi ve içerdiği kodu yürütmeyi deneyebilir.
  
Ağ senaryosunda, saldırgan bir UNC veya WebDAV konumuna bir meşru dosya ve özel hazırlanmış bir DLL dosyası yerleştirebilir ve kullanıcıyı bu meşru dosyayı açmaya ikna edebilir.
...
Güncelleştirme, Windows BranchCache'in dış kitaplıkları yükleme biçimini düzelterek bu güvenlik açığını giderir.
...
<td style="border:1px solid black;">Bkz: <a href="http://support.microsoft.com/kb/2385678">Microsoft Bilgi Bankası makalesi 2385678</a></td>
</tr>
<tr class="odd">
...
-   [Fortinet bünyesindeki FortiGuard Labs](http://www.fortiguard.com/) için çalışan Haifei Li, BranchCache'de Güvenli Olmayan Şekilde Kitaplık Yükleme Güvenlik Açığı'nı (CVE-2010-3966) bildirdiği için
